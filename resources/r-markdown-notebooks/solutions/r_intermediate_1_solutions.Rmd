...
# your code here
pairings <- rep(NA, length(animals))

for (pets in seq_along(1:length(animals))) {
  pairings[pets] <- paste(people[pets], "owns a", animals[pets])
}
...
for (age in seq_along(ages)) {
  if (ages[age] < 17) {
    print(paste0("Age ", ages[age], ": can't drive, breaking loop"))
    break
  }
  print(paste0("Age ", ages[age], ": can drive"))
}
print("Loop finished")
...
for (age in seq_along(ages)) {
  if (ages[age] < 17) {
    print(paste0("Age ", ages[age], ": can't drive, moving on"))
    next
  }
  print(paste0("Age ", ages[age], ": can drive"))
}
...
penguins_stats <- data.frame(
  medians = rep(NA, length(cols)),
  sds = rep(NA, length(cols)),
  iqrs = rep(NA, length(cols))
)
...
for (stats in seq_along(penguins[, cols])){
  ...
  penguins_stats[stats, "medians"] <- median(penguins[, cols[stats], drop = TRUE], na.rm = TRUE)
  # sd
  penguins_stats[stats, "sds"] <- sd(penguins[, cols[stats], drop = TRUE], na.rm = TRUE)
  # IQR
  penguins_stats[stats, "iqrs"] <- IQR(penguins[, cols[stats], drop = TRUE], na.rm = TRUE)
}
...
# your code here
les_mis <- subset(broadway_grosses, show == "Les Miserables")

cols <- c("weekly_gross", "avg_ticket_price", 
                          "pct_capacity", "performances")

# filtered data by columns
les_mis_filt <- les_mis[, cols]

# make plotting grid
par(mfrow = c(2,2))

# for each column, make a plot
for (col in colnames(les_mis_filt)) {
  x <- les_mis_filt[[col]]
  hist(x, main = col, xlab = col)
}
```
...
1.  Subset your `broadway_grosses` data to just contain the shows classic shows listed above
...
# solution! 
selected_shows <- c("The Phantom of the Opera", "Chicago", "The Lion King", "Les Miserables")

top_shows <- subset(broadway_grosses, show %in% selected_shows)

par(mfrow = c(2,2))

for (show_name in selected_shows) {
  x <- subset(top_shows, show == show_name)
  plot(avg_ticket_price ~ pct_capacity,
       data = x,
       main = unique(show),
       pch = 19)
}
...
<https://lse.eu.qualtrics.com/jfe/form/SV_2beTiFd70AlRe5M?coursename=R%Intermediate%1:%Introduction%to%for%loops&topic=R&prog=DS&version=23-24&link=https://lsecloud.sharepoint.com/:u:/s/DSL-R-Presessional9-SolutionsandTrainerMaterials/EZySoRQZyTVFrPu9Gn7bGyAB07yCFNm1qoLQOMtPtPuNkw?e=mnHVpc>
...
for (sheetname in seq_along(sheets)) {
  broadway[[sheetname]] <- read_excel(path, sheet = sheets[[sheetname]])
}
...
top_broadway <- do.call(rbind, broadway)
```

Note, have only have 4 datasets here so we can do this manually. However, if this was to be expanded this would quickly become tiresome! 
...
for (col in seq(2,4, by = 1)) {
  plot(top_broadway_filt[, c(col, 1)])
}
...
for (i in 2:ncol(top_broadway_filt)) {
  predictors_i <- colnames(top_broadway_filt)[2:i]
  model_summaries[[i - 1]] <- summary(lm(weekly_gross_overall ~ ., 
       top_broadway_filt[ , c("weekly_gross_overall", predictors_i)]))
}

model_summaries

# different ways of reviewing outputs
# using loop to look at adjusted R Square of each model
for (i in seq_along(model_summaries)) {
  r_sq <- round(model_summaries[[i]][["adj.r.squared"]], 3)
  print(paste0("Model ", i," has adjusted R Squared of ", r_sq))
}

# load in broom for looking at results
library(broom)

# using broom tidy
broom::tidy(model_summaries[[3]])

# using broom glance
model_glance <- list()

for (i in seq_along(model_summaries)) {
  model_glance[[i]] <- broom::glance(model_summaries[[i]])
}

model_sum_df <- do.call(rbind, model_glance)
model_sum_df$model <- c("model 1", "model 2", "model 3")
model_sum_df <- model_sum_df[, c(ncol(model_sum_df), 1:(ncol(model_sum_df) - 1))]
