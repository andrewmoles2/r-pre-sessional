...
# your code here
perc_phrase <- function(data, col = "Name", phrase = "saurus") {
  x = (sum(grepl(phrase, data[[col]], ignore.case = TRUE)) / nrow(data)) * 100
  return(x)
}

perc_phrase(dinosaurs)
perc_phrase(dinosaurs, phrase = "titan")
perc_phrase(dinosaurs, "Class", phrase = "poda")
...
```{r}
# your code here
count_missing <- function(df) sum(is.na(df))
...
BMI <- function(weight, height, unit = "m") {
  weight <- weight
  if(unit == "m") { 
    height <- height 
  } else if(unit == "cm") { 
    ...
    return("please select a valid unit: centimetres, meters or feet")
  }
  BMI <- weight/(height^2)
  return(BMI)
}
...
# alternate solution using match.arg and switch
# see: https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/match.arg
bmi <- function(weight, height, unit = c("centimetres", "meters", "feet")) {
  unit <- match.arg(unit)
  switch(
    unit,
    meters = weight/(height^2),
    centimetres = weight/((height/100)^2),
    feet = weight/((height/3.28)^2)
  )
}
...
generation <- function(birth_year) {
  if(birth_year >= 1997 & !is.na(birth_year)){
    gen <- "Gen Z"
  }
  else if(birth_year <= 1996 & birth_year >= 1981 & !is.na(birth_year)){
    gen <- "Millennial"
  }
  else if(birth_year <= 1980 & birth_year >= 1965 & !is.na(birth_year)){
    gen <- "Gen X"
  }
  else if(birth_year <= 1964 & birth_year >= 1955 & !is.na(birth_year)){
    gen <- "Boomer II"
  }
  else if(birth_year <= 1954 & birth_year >= 1946 & !is.na(birth_year)){
    gen <- "Boomer I"
  }
  else if(birth_year <= 1945 & birth_year >= 1928 & !is.na(birth_year)){
    gen <- "Post War"
  }
  else if(birth_year <= 1927 & birth_year >= 1922 & !is.na(birth_year)){
    gen <- "WW II"
  }
  else {
    gen <- "Unknown"
...
# Applying in data frame

family$generation <- sapply(family$year, generation)
family
```
...
<https://lse.eu.qualtrics.com/jfe/form/SV_2beTiFd70AlRe5M?coursename=R%Intermediate%3:%Making%functions&topic=R&prog=DS&version=23-24&link=https://lsecloud.sharepoint.com/:u:/s/DSL-R-Presessional9-SolutionsandTrainerMaterials/Ee9wz_qauTNDrVPemVLN0WUBTMiNcH0fETlLydvJs86moQ?e=Uw28gN>
...
BMI2 <- function(weight, height, units = "m") {
  weight <- weight
  if(units == "m") { height <- height 
  } else if(units == "cm") { 
    height <- height/100
  } else if(units == "ft") { 
    height <- height/3.28
  } else {
    return("please select a valid unit: cm, m or ft")
  }
  BMI <- weight/(height^2)
...
GP$Obesity <- mapply(BMI2, GP$Weight, GP$Height, GP$Unit)
purrr::pmap_chr(list(GP$Weight, GP$Height, GP$Unit), BMI2)

message <- function(name, condition){
  if (condition== "obese"){
  message <- paste0("Dear ", name, ", our information tell us you are eligible for a COVID-19 booster shot, please feel welcome to go to your GP as soon as possible to receive it")
  print(as.character(message))
  }
 return(0) 
}

GP$Message <- mapply(message, GP$Names, GP$Obesity)
purrr::map2(GP$Names, GP$Obesity, message)
